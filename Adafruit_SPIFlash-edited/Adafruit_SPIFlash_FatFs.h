...
public:
  FlashFile():
    ...
    _fatfs(NULL)
  {}
  FlashFile(const char* filepath, uint8_t mode = FILE_READ,
...
// Serial.println((unsigned int)_dirPath,HEX);
  ...
  FlashFile openNextFile(uint8_t mode = (FA_READ | FA_OPEN_EXISTING));
...
public:
  ...
  bool begin();
  ::FlashFile open(const char *filename, uint8_t mode = FILE_READ);
  ::FlashFile open(const String &filename, uint8_t mode = FILE_READ) {
