...
FlashFile::FlashFile(const char* filepath, uint8_t mode, Adafruit_SPIFlash_FatFs* fatfs):
...
// Serial.println(filepath);
  ...
  if (f_stat(filepath, &_fileInfo) != FR_OK) {
    // FlashFile doesn't exist, but should it? (i.e. we're not writing).
    if ((mode & FA_READ) == 0) {
  ...
  if ((_fileInfo.fattrib & AM_DIR) > 0) {
    ...
    for (int i = 0;i<pathLength;i++)
	    _dirPath[i]=filepath[i];
    _dirPath[pathLength] = 0;
// Serial.print("BLW Y ");
// Serial.print(_dirPath);
// Serial.print(" 0x");
// Serial.println((unsigned int)_dirPath,HEX);
...
size_t FlashFile::write(const uint8_t *buf, size_t size) {
  activate();
  UINT bw = 0;
...
int FlashFile::read(void *buf, uint16_t nbyte) {
  activate();
  UINT br = 0;
...
bool FlashFile::seek(uint32_t pos) {
  activate();
  return f_lseek(&_file, pos) == FR_OK;
}

uint32_t FlashFile::position() {
  activate();
  return f_tell(&_file);
}
...
FlashFile FlashFile::openNextFile(uint8_t mode) {
  activate();
...
FlashFile MyFlashFile = FlashFile(path, mode, _fatfs);
  return MyFlashFile;
}

void FlashFile::rewindDirectory() {
  activate();
...
FlashFile Adafruit_SPIFlash_FatFs::open(const char *filename, uint8_t mode) {
  activate();
  return FlashFile(filename,  mode, this);
}
...
bool Adafruit_SPIFlash_FatFs::rmdir(const char *filepath) {
  ...
  FlashFile child = root.openNextFile();
  while (child) {
